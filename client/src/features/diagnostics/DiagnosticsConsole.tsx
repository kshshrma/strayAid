...
export const DiagnosticsConsole = () => {
  ...
  const triggerTestRoute = (nameParam: string) => {
    ...
    fetch(`/api/test?name=${nameParam}`)
      .then(async (res) => {
        ...
        if (!res.ok) {
          // If response not 2xx, return parsed JSON error response
          return {
            success: false,
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white border border-neutral-200/80 rounded-2xl shadow-sm p-6 sm:p-8 text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-neutral-100 pb-6 mb-6 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-500 font-medium">Animal Emergency Operating System • Diagnostics Console</p>
          </div>
        </div>
      ...
      {/* Footer */}
      <div className="border-t border-neutral-100 mt-6 pt-4 flex flex-col sm:flex-row items-center justify-between text-xs text-neutral-400 gap-2">
        <span>StrayAid AEOS Diagnostic Console • Local Developer Mode</span>
        <span className="font-semibold text-primary-600">Phase 1 Diagnostic Complete</span>
      </div>
